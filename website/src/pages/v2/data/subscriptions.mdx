...
The `subscribeToMore` is a bit different than `subscribe`. It is a method available on every watched query in `apollo-angular`. It works just like [`fetchMore`](../caching/interaction#incremental-loading-fetchmore), except that the update function gets called every time the subscription returns, instead of only once.
...
<Callout>
  You can use `connectionParams` for anything else you might need, not only authentication, and check its payload on the
  server side with [SubscriptionsServer](https://www.apollographql.com/docs/graphql-subscriptions/authentication/).
</Callout>
