---
description: Automatic Persisted Queries for Apollo Angular. Improve GraphQL performance with Automatic Persisted Queries.
---
...
In practice we've seen GraphQL query sizes ranging well above 10 KB just for the query text. This is
significant overhead when compared with a simple URL of 50-100 characters. When paired with the fact
that the uplink speed from the client is typically the most bandwidth-constrained part of the chain,
large queries can become bottlenecks for client performance.
...
If you do not already have a SHA-256 based hashing function available in your application, you will
...
This link doesn't include a SHA-256 hash function by default, to avoid forcing one as a dependency.
