...
Apollo Client uses the ultra flexible [Apollo Link](https://apollographql.com/docs/link) that includes several options for authentication.
...
<Callout type="warning">
  **Note**: the backend must also allow credentials from the requested origin. e.g. if using the popular `cors` package
  from NPM in Node.js.
</Callout>
