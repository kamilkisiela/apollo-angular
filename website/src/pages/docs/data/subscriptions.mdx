...
function apolloOptionsFactory(): ApolloClientOptions<any> {
  ...
  Socket link:
  const ws = n
  const ws = new GraphQLWsLink(createClient({ url: 'ws://localhost:3000/graphql' }));
  ility to split links, you can send data to each link
  // depending
   what kind of operation is being sent
  const link =
  const link = split(
    on operation type
    ({query})
