...
<Callout>
  Field policies are primarily documented in [Customizing the behavior of cached fields](../caching/field-behavior).
  This article specifically describes how to use them with local-only fields.
</Callout>
...
<Callout>
  If you query a local-only field that _doesn't_ define a `read` function, Apollo Client performs a default cache lookup
  for the field. See [Storing local state in the cache](#storing-local-state-in-the-cache) for details.
</Callout>
...
<Callout>
  ...
  execution order must not affect the result, and hence the server has the freedom to execute the field entries in
  whatever order it deems optimal.
</Callout>
