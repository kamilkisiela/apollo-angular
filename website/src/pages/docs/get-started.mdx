...
In `graphql.provider.ts` file use `APOLLO_OPTIONS` token to configure Apollo:
...
Finally, the `graphqlProvider` must be used somewhere and the `HttpLink` must be provided too. This
happens in `app.config.ts`:
...
export const appConfig: ApplicationConfig = {
  providers: [
    provideHttpClient(),
    graphqlProvider, r providers for your application ...
  ]
};
  ],
};
