---
description: Apollo Angular is a community-maintained Angular integration for Apollo Client. It provides you with everything you need to use Apollo Client in your Angular app.
---
...
[**let's go through it together here**](#request-data)
...
npm i apollo-link apollo-client apollo-cache-inmemory
...
In our `app.module.ts` file use `ApolloModule` and `APOLLO_OPTIONS` token to configure Apollo
Client:
...
1. With `apollo-angular-link-http` and `HttpLink` service we connect our client to an external
   GraphQL Server
...
Apollo's HttpLink requires `HttpClient` so that's why we also used `HttpClientModule` from
`@angular/common/http`.
...
One Link that you already have in your application is called `apollo-angular-link-http` which uses
HTTP to send your queries.

`apollo-cache-inmemory` is the default cache implementation for Apollo Client 2.0. InMemoryCache is
a normalized data store that supports all of Apollo Client 1.0’s features without the dependency on
Redux.

- [Explore more the Network Layer of Apollo](./network-layer)
- [Read more about caching](./caching)
...
First, pass your GraphQL query wrapped in the `gql` function (from `graphql-tag`) to the `query`
...
- [Queries](./queries): Learn how to fetch queries with arguments and dive deeper into configuration
  options..
- [Mutations](./mutations): Learn how to update data with mutations and when you'll need to update
  the Apollo cache.
