---
description: This guide will teach you how to use Apollo Client with Angular to build a GraphQL-powered app.
---
...
1. The mutation name with arguments (`submitRepository`), which represents the actual operation to
   be done on the server
1. The fields you want back from the result of the mutation to update the client (`id` and
   `repoName`)
...
[based on the id of the result](../features/cache-updates#normalization-with-dataidfromobject),
which in turn updates UI automatically, so we don't explicitly handle the results ourselves. In
order for the client to correctly do this, we need to ensure we select the correct fields (as in all
the fields that we care about that may have changed).
...
<Callout>
  Note that in general you shouldn't attempt to use the results from the mutation callback directly,
  instead you can rely on Apollo's id-based cache updating to take care of it for you, or if
  necessary passing an [`updateQueries`](../features/cache-updates#updatequeries) callback to update
...
<Callout>
  ...
  [`updateQueries`](../features/cache-updates#updatequeries) to make it appear in this case (and
  this is what we do in GitHunt).
</Callout>
