...
`subscribeToMore` is a method available on every query in `apollo-angular`. It works just like [`fetchMore`](./cache-updates#incremental-loading-fetchmore), except that the update function gets called every time the subscription returns, instead of only once.
