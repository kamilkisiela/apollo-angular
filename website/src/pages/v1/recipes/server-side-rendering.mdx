---
description: Learn how to use Apollo with Angular to build a GraphQL-powered app. This tutorial covers everything from setting up Apollo Client to querying data with GraphQL.
---
...
class AppModule {
  ...
  constructor(apollo: Apollo, httpLink: HttpLink, private readonly transferState: TransferState) {
    this.cache = new InMemoryCache();
...
About `isBrowser`, `this.transferState.hasKey<any>(STATE_KEY)` will return a value only if it runs
...
In `onServer` we extract data from cache when TransferState says it is ready to save and serialize
data. In `OnBrowser` we do opposite thing, we receive transfered data to restore cache.
...
1. When [creating Apollo](../basics/setup) (`Apollo.create`) on the server, you'll need to set up
...
1. You need to ensure that you create a new client or store instance for each request, rather than
   ...
   you'll have problems with [authentication](./authentication).
