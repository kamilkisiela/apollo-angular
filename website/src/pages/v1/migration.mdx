...
<Callout>
  **Note:** If you were using `customResolvers`, the name of that has been changed to be `cacheResolvers` to be more
  descriptive of what it does. `customResolvers` will still be supported throughout the 2.0 though to be backwards
  compatible and ease the upgrade path.
</Callout>
...
For more information on using Apollo Links, check out the [link docs](https://apollographql.com/docs/link)!
